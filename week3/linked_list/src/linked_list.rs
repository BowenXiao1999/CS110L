...
impl<T> Clone for LinkedList<T> where Node<T>: Clone{
...
impl<T> PartialEq for LinkedList<T> where Node<T>: PartialEq {
...
// impl<T> IntoIterator for &LinkedList<T> where T: Clone {
//     type Item = Option<Box<Node<T>>>;
//     type IntoIter =  LinkedList<T>::IntoIterator(Self::Item);
